...
export default function DependencyPage({ projectName, version }) {
  ...
  const fetchRows = useCallback(async () => {
    try {
      setLoading(true);
      const data = await dependencyApi.getAll(projectName, version);
      setRows(data);
      setError(null);
    } catch {
  ...
  const handleAddRow = async () => {
    try {
      const newRow = await dependencyApi.create('New dependency', projectName, version);
  ...
  const handleInsertAfter = async (afterIndex) => {
    setContextMenu(null);
    try {
      await dependencyApi.create('New dependency', projectName, version);
      const data = await dependencyApi.getAll(projectName, version);
      setRows(data);
  ...
  const handleUpdateRow = async (id, field, value) => {
    ...
    try {
      await dependencyApi.update(id, value);
        ...
        <table className="rg-table">
          <thead>
            <tr>
              ...
              <th className="rg-th">Dependency</th>
